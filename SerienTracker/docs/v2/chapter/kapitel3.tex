...
\begin{lstlisting}[label=xsd-definition-episode,caption= Definition des complexElement Episode mit Elementen und Attributen]
...
\begin{lstlisting}[label=xsd-definition-master,caption= Auszug aus der Masterinklude Serientracker.xsd]
...
Durch die Definition einzelner Schemas erhält jedes Objekt dieses Typs auch eine eigene XML Datei. Im späteren Verlauf könnten sich daraus aber Probleme beim gezielten Zugriff entwicklen, denen frühzeitig Abhilfe geschaffen werden sollte.\\
...
\begin{lstlisting}[label=xsd-definition-series,caption= Auszug aus der Series.xsd Definition]
...
\begin{lstlisting}[label=xsd-definition-idtype,caption= Definition der globalen IDs]
...
\begin{lstlisting}[label=seriesservice,caption= GET Testmethode der SeriesID]
   @Path( "/{id}" )
